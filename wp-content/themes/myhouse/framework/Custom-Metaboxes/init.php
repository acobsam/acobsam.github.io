...
class cmb_Meta_Box {
	...
	public static function autoload_helpers( $class_name ) {
		...
		$file = "$dir/framework/Custom-Metaboxes/helpers/$class_name.php";
		if ( file_exists( $file ) )
			@include( $file );
	}
	...
	public function register_scripts() {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => esc_html__('Clear' ,'myhouse'),
					'defaultString' => esc_html__('Default' ,'myhouse'),
					'pick'          => esc_html__('Select Color','myhouse' ),
					'current'       => esc_html__('Current Color' ,'myhouse'),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'upload_file'     => esc_html__('Use this file', 'myhouse'),
			'remove_image'    => esc_html__('Remove Image', 'myhouse'),
			'remove_file'     => esc_html__('Remove', 'myhouse'),
			'file'            => esc_html__('File:', 'myhouse'),
			'download'        => esc_html__('Download', 'myhouse'),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
			'up_arrow'        => esc_html__('[Up]&nbsp;', 'myhouse'),
			'down_arrow'      => esc_html__('&nbsp;[ Down ]', 'myhouse'),
			'check_toggle'    => esc_html__('Select / Deselect All', 'myhouse' ),
		) ) );
	...
	public function add_post_enctype() {
		...
		jQuery(document).ready(function(){
			jQuery("#'. esc_attr($this->form_id) .'").attr("enctype", "multipart/form-data");
			jQuery("#'. esc_attr($this->form_id) .'").attr("encoding", "multipart/form-data");
		});
		</script>';
	}
	...
	public static function render_group( $args ) {
		...
		if ( $desc || $label ) {
			echo '<tr><th>';
				...
				if ( $desc )
					echo '<p class="cmb_metabox_description">'. esc_attr($desc) .'</p>';
			echo '</th></tr>';
		}
		...
		echo '<tr><td><p class="add-row"><button data-selector="', esc_attr($field_group->id()) ,'_repeat" data-grouptitle="', esc_attr($field_group->options( 'group_title' )) ,'" class="add-group-row button">'. esc_attr($field_group->options( 'add_button' )) .'</button></p></td></tr>';
				...
				echo '
					<tr>
						<td class="remove-row" colspan="2">
							<button '. esc_attr($remove_disabled) .'data-selector="'. esc_attr($field_group->id()) .'_repeat" class="button remove-group-row alignright">'. esc_attr($field_group->options( 'remove_button' )) .'</button>
						</td>
					</tr>
				</table>
			</td>
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$content_url = str_replace( $content_dir, WP_CONTENT_URL, get_template_directory());
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__('Save','myhouse' ) );
