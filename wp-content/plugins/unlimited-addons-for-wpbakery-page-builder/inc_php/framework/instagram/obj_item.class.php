...
class InstaObjItemUC{
	...
	public function initNewAPI($item){
		...
		$this->createdDateStamp = UniteFunctionsUC::getVal($item, "date");
		...
		$this->arrImages[self::IMAGE_STANDARD]["url"] = UniteFunctionsUC::getVal($item, "display_src");
		
		$arrLikes = UniteFunctionsUC::getVal($item, "likes");
		$this->numLikes = UniteFunctionsUC::getVal($arrLikes, "count"); 
		
		//init owner
		$owner = UniteFunctionsUC::getVal($item, "owner");
		if(!empty($owner)){
			$this->itemUser = new InstaObjUserUC();
			$this->itemUser->initByNew($owner);
		}
